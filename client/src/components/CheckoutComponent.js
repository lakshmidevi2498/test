...
const CheckoutComponent = () => {
  ...
  useEffect(() => {
    const fetchCheckoutProducts = async () => {
      ...
      console.log("userIdproductId", userId ,token);
      if (userId && token) {
        dispatch(loadCheckoutInitiate(token,userId))
      }
      else {
        console.log("error")
      }
  ...
  const handleRemoveFromCheckout = async (productId) => {
    ...
    console.log("userIdproductId", userId, productId ,token);

    try {
      if (userId && token) {
        await dispatch(deleteCheckoutInitiate(token,userId, productId))
